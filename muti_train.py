...
from torch.nn import functional as F, DataParallel  # 从PyTorch的神经网络模块中导入函数式API和DataParallel
...
def parse_args():
    ...
    parser.add_argument("--work_dir", type=str, default="workdir", help="工作目录")  # 添加工作目录参数
    ...
    parser.add_argument("--data_path", type=str, default="dataset/new_isic2017", help="数据集路径")  # 添加数据集路径参数
    parser.add_argument("--metrics", nargs='+', default=['iou', 'dice'], help="评估指标")  # 添加评估指标参数
    parser.add_argument('--device', type=str, default='cuda', help="运行设备")  # 修改为cuda以支持多GPU
    parser.add_argument("--lr", type=float, default=1e-3, help="学习率")  # 添加学习率参数
...
def getModel(args):
    ...
    if args.run_name == 'resnet34_unet':  # 4G
        model = resnet34_unet(1, pretrained=True)
    ...
    if args.run_name == 'mamba_sam':
        model = SegModel(out_chans=1024, freeze_encoder=False, input_size=args.image_size, deep_supervision=True)
...
def plot_result(train_data, test_data, description, save_name, save_path, run_name, current_time, data_path):
    # 计算测试数据中的最佳值
    best_test_value = max(test_data)
    # 生成x轴刻度
    x_ticks = np.arange(len(train_data))
    # 绘制训练和测试数据曲线
    plt.plot(x_ticks, train_data, label='Train')
    plt.plot(x_ticks, test_data, label='Test')
    # 设置标题，包含测试数据中的最佳值
    if description == 'Loss':
        plt.title(f'{description}  batch_size:{args.batch_size} lr:{args.lr}')  # 设置图表标题
    else:
        plt.title(f'{description} (Best Test: {best_test_value:.4f} batch_size:{args.batch_size} lr:{args.lr})')
    # 设置x轴和y轴标签
    plt.xlabel('Epoch')
    ...
    # 使用更新的文件名保存图像
    filename = f'{save_name}_{run_name}_{data_path}_{current_time}.png'
...
def main(args):
    ...
    if args.resume is not None:
        with open(args.resume, "rb") as f:
            ...
            if torch.cuda.device_count() > 1:
                model.module.load_state_dict(checkpoint['model'])  # 加载模型权重
            else:
    ...
    for epoch in range(0, args.epochs):
        ...
        if float(test_metrics['iou']) > best_iou:
            ...
            state = {'model': model.module.float().state_dict(),
                     'optimizer': optimizer} if torch.cuda.device_count() > 1 else {'model': model.float().state_dict(),
                                                                                    'optimizer': optimizer}  # 创建保存的模型状态
            torch.save(state, save_path)  # 保存模型

        end = time.time()  # 记录结束时间
        print("Run epoch time: %.2fs" % (end - start))  # 打印本次epoch运行时间

        plot_result(train_loss, test_loss, 'Loss', 'Loss', args.work_dir, args.run_name, current_time,
                    args.data_path)  # 绘制并保存训练和测试损失曲线
        plot_result(train_dices, test_dices, 'Dice', 'Dice', args.work_dir, args.run_name, current_time,
                    args.data_path)  # 绘制并保存训练和测试Dice系数曲线
        plot_result(train_ious, test_ious, 'Iou', 'Iou', args.work_dir, args.run_name, current_time,
                    args.data_path)  # 绘制并保存训练和测试Dice系数曲线
